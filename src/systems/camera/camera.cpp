...
namespace sys::cam {
    void CameraMovement(Storage::Registry& world, Camera3D& camera, const float delta_time, const bool shake_cam) {
     ...
     void CameraMovement_Static(Storage::Registry& world, Camera3D& camera, const float delta_time, const bool shake_cam) {
        PROFILE_SCOPE("CameraMovement()");
        Vector3 position = Vector3Zero();
         for (auto entity : world.View<tag::CameraFollow, cmpt::Transform>()) {
            auto& trans = world.GetComponent<cmpt::Transform>(entity);
            position = trans.position;
            break;
        }
